...
dist.mat <-sim(SiteXSpecies,  method='simpson')
...
for (i in cvi) {
  ...
  vals[[i]] <- NbClust(data = SiteXSpecies, diss = dist.mat, distance = NULL, 
...
#2, 3, 7, 9, 12 clusters all possible

saveRDS(p.cvi, 'Output/CVI_Gulf.rds')

# ggsave(plot = p.cvi, "Output/CVI_Gulf.tiff", height = 4, width = 6, units = 'in', dpi = 300, compression = 'lzw')
...
table$distance_Bsim[table$nClusters>=3 & table$nClusters<=12] #Look for cut-off between 0.480-0.645
...
#Area plot
#####
tiff("Output/CutOff_Bsim_byNSites_Gulf.tiff",
...
ggplot(table2) +
  ...
  scale_y_continuous("Number of sites in each cluster", limits=c(0,1350), expand=c(0,0))+labs(fill="") +
...
#####
#
 tiff("Output/CutOff_NSitesBySD_Gulf.tiff",
...
points(tablea$rowsums[tablea$Bsim100>=1&tablea$Bsim100<=31],tablea$SD[tablea$Bsim100>=1&tablea$Bsim100<=31], type="l",  lty=5, lwd=2)
...
legend("topleft",legend=c((expression(paste(beta["sim"]," cut-off"))),"< 0.30","0.30 to 0.39","0.40 to 0.49","0.50 to 0.59", "0.60 to 0.69"),lty=c(0,5,4,1,2,1),lwd=c(0,2,2,1,1.5,2), bty="n")
points(1326,276.6948,col="black", pch=22, cex=3)
# points(tablea$rowsums[tablea$Bsim==cutoff1],tablea$SD[tablea$Bsim==cutoff1],col="black", pch=21, cex=3)
text(1326-125,276.6948,col="black", labels=c("0.498"))
