...
table$distance_Bsim[table$nClusters>=2 & table$nClusters<=11] #Look for cut-off between 0.473-0.698
...
#Area plot
#####
tiff("Output/CutOff_Bsim_byNSites_QC.tiff",
...
ggplot(table2) +
  ...
  scale_x_continuous(expression(paste(beta["sim"]," cut-off")), limits = c(0,1),expand = c(0, 0))+
  scale_y_continuous("Number of sites in each cluster", limits=c(0,1600), expand=c(0,0))+labs(fill="") +
...
#####
#
 tiff("Output/CutOff_NSitesBySD_QC.tiff",
...
points(tablea$rowsums[tablea$Bsim100>=0&tablea$Bsim100<=31],tablea$SD[tablea$Bsim100>=0&tablea$Bsim100<=31], type="l",  lty=5, lwd=2)
...
legend("topleft",legend=c((expression(paste(beta["sim"]," cut-off"))),"< 0.30","0.30 to 0.39","0.40 to 0.49","0.50 to 0.59"),lty=c(0,5,4,1,2),lwd=c(0,2,2,1,1.5), bty="n")
points(1580,348.1775,col="black", pch=22, cex=3)
# points(tablea$rowsums[tablea$Bsim==cutoff1],tablea$SD[tablea$Bsim==cutoff1],col="black", pch=21, cex=3)
text(1580-150,348.1775,col="black", labels=c("0.499"))
